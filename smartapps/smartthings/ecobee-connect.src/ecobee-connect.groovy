 ...
 *	0.9.20 - Custom Home66 Integration
 *
 */  
def getVersionNum() { return "0.9.21" }
private def getVersionLabel() { return "Ecobee (Connect) HJ Version ${getVersionNum()}" }
...
def setProgramTemp(child, program, deviceId, temp) {
	// NOTE: Will set the first program matching the supplied name
	LOG("setProgram() ${program} temperature to ${temp} with DeviceID: ${deviceId}", 4, child)    
    
	def therm = atomicState.thermostatData?.thermostatList?.find { it.identifier.toString() == deviceId.toString() }
    def programData = therm?.program
    def climates = programData?.climates
    def climate = climates?.find { it.name.toString() == program.toString() }
    LOG("*********************programData - {$programData}", 4, child)
    LOG("climates - {$climates}", 4, child)
    LOG("climate - {$climate}", 4, child)
    def climateRef = climate?.climateRef.toString()
	
    if (climate == null) { 
    	LOG("Climate = null")
    	return false }
    def formattedClimates = '['
    therm?.program?.climates?.each {
    	if ( it.climateRef.toString() == climateRef)
        {
        	def climateTemp = it.heatTemp.toString()
        	LOG("***************ClimateTemp - {$climateTemp}", 4, child)
        	it.heatTemp = temp
            climateTemp = it.heatTemp.toString()            
        	LOG("***************NewClimateTemp - {$climateTemp}", 4, child)
        }
        formattedClimates += it.toString()+','
    }
    formattedClimates = formattedClimates.substring(0, formattedClimates.length() - 1)+']'
	LOG("***************formattedClimates - $therm?.program?.climates.toString()", 4, child)
	def jsonRequestBody
    jsonRequestBody = '{"selection":{"selectionType":"registered","selectionMatch":""},thermostat:{"program":{"schedule":'+therm?.program?.schedule+',"climates":'+formattedClimates+'}}}'
